...
public class BatCavernDialogue {
	...
	public static final DialogueNode CAVERN_ATTACK = new DialogueNode("敌袭！", "一个家伙从阴影中跳了出来！", true) {
		@Override
			...
			if(isWantsToFight()) {
				if (index == 1) {
					...
					if(Main.game.getPlayer().getMoney()<Main.game.getDialogueFlags().getMuggerDemand2()) {
						...
						return new Response("给钱("+UtilText.formatAsMoney(Main.game.getDialogueFlags().getMuggerDemand2(), "span")+")",
								"给了[npc.Name]"+Util.intToString(Main.game.getDialogueFlags().getMuggerDemand2())+"火币，你最终得以脱身。", Main.game.getDefaultDialogue(false)) {
							@Override
							public void effects() {
								applyPregnancyReactions();
					...
					if(!Main.game.getPlayer().hasItemType(ItemType.MUSHROOM)) {
						return new Response("交出蘑菇", "你没有任何可以给予[npc.name]的蘑菇", null);
					} else {
						return new Response("给出蘑菇", "拿一个发光蘑菇给[npc.name]，让[npc.her]离开你。", Main.game.getDefaultDialogue(false)) {
							@Override
							public void effects() {
								if(getMugger().isVisiblyPregnant()){
									getMugger().setCharacterReactedToPregnancy(Main.game.getPlayer(), true);
				...
				if (index == 1) {
					return new Response("对话", "和[npc.name]聊一会儿，以便于多了解[npc.herHim]一点。", CAVERN_PEACEFUL_TALK) {
						@Override
						public void effects() {
							applyPregnancyReactions();
					...
					if(Main.game.getPlayer().getMoney()<Main.game.getDialogueFlags().getMuggerDemand2()) {
						return new Response("给钱("+UtilText.formatAsMoney(Main.game.getDialogueFlags().getMuggerDemand2(), "span")+")",
								"你没有足够的钱可以给[npc.name]。", null);
					} else {
						return new Response("给钱("+UtilText.formatAsMoney(Main.game.getDialogueFlags().getMuggerDemand2(), "span")+")",
								"给[npc.name]一些钱，让[npc.herHim]能够买一些食物和衣服。", CAVERN_PEACEFUL_OFFER_MONEY) {
							@Override
							public void effects() {
								applyPregnancyReactions();
					...
					if(!Main.game.getPlayer().hasQuest(QuestLine.SIDE_ACCOMMODATION) || !getMugger().isAffectionHighEnoughToInviteHome()) {
						...
						return new Response("提供房间", "问一问[npc.name]想不想在莉莱雅的宅邸中要一个房间。", CAVERN_PEACEFUL_OFFER_ROOM) {
							@Override
							public void effects() {
								applyPregnancyReactions();
					...
					return new Response("攻击", "背叛[npc.namePos]的信任并攻击[npc.herHim]！", CAVERN_PEACEFUL_ATTACK) {
						@Override
						public void effects() {
							applyPregnancyReactions();
					...
					if(!getMugger().isAttractedTo(Main.game.getPlayer())) {
						...
						return new ResponseSex(UtilText.parse(companion, "三人行"),
								UtilText.parse(getMugger(), companion, "给[npc.name]一个和你，以及[npc2.name]一起做爱的机会。"),
								true, false,
								new SMGeneric(
										Util.newArrayListOfValues(getMugger()),
										Util.newArrayListOfValues(Main.game.getPlayer(), companion),
	...
	public static final DialogueNode CAVERN_PEACEFUL_TALK = new DialogueNode("对话", "", true, true) {
	...
	public static final DialogueNode CAVERN_PEACEFUL_OFFER_MONEY = new DialogueNode("献上财物", "", true, true) {
	...
	public static final DialogueNode CAVERN_PEACEFUL_OFFER_ROOM = new DialogueNode("提供房间", "", true, true) {
		...
		public Response getResponse(int responseTab, int index) {
			if (index == 1) {
				return new Response("带回家", "带[npc.name]到[npc.her]的新房间。", CAVERN_PEACEFUL_OFFER_ROOM_BACK_HOME) {
					@Override
					public void effects() {
	...
	public static final DialogueNode CAVERN_PEACEFUL_OFFER_ROOM_BACK_HOME = new DialogueNode("新房间", "", true) {
	...
	public static final DialogueNode CAVERN_PEACEFUL_ATTACK = new DialogueNode("攻击", "", true, true) {
	...
	public static final DialogueNode AFTER_COMBAT_VICTORY = new DialogueNode("胜利", "", true) {
		...
		public Response getResponse(int responseTab, int index) {
			...
			if (index == 1) {
				...
				return new ResponseEffectsOnly("物品栏", "现在你已经打败了[npc.name]，没什么能够阻止你向[npc.her]的衣服和道具出手……"){
					@Override
					public void effects() {
				...
				if(!Main.game.isNonConEnabled() && (!getMugger().isAttractedTo(Main.game.getPlayer()) || !getMugger().isAttractedTo(companion))) {
					...
					return new Response(UtilText.parse(companion, "三人行"), UtilText.parse(companion, getMugger(), "[npc.Name]并没有被[npc2.name]所吸引，所以不愿意与[npc2.herHim]发生性关系！"), null);
			...
			if((!forcedTF || potion==null)
				...
				if (index == 1) {
					...
					if(Main.game.getPlayer().hasFetish(Fetish.FETISH_TRANSFORMATION_RECEIVING)) {
						...
						return new Response("吐出", "吐掉药水。", AFTER_COMBAT_TRANSFORMATION) {
							@Override
							public void effects() {
								transformationsApplied = true;
								Main.game.getTextStartStringBuilder().append(UtilText.parseFromXMLFile("encounters/submission/batCavern/"+getDialogueId(), "TF_SPIT", getAllCharacters()));
