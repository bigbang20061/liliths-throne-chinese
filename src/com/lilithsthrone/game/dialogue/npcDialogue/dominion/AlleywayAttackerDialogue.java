...
public class AlleywayAttackerDialogue {
	...
	private static Response getUnbirthSwallowResponse() {
		if(!Main.game.isUnbirthContentEnabled()) {
			return null;
		}
		if(!Main.game.canBeSwallowed(getMugger())) {
			return new Response("吞入",
					UtilText.parse(getMugger(), Main.game.getSwallowBlockReason(getMugger())),
					null);
		}
		return new Response("吞入",
				UtilText.parse(getMugger(), "将[npc.name]纳入你的体内，让[npc.herHim]待在你的子宫之中。"),
				DialogueManager.getDialogueFromId("unbirth_scenes_swallow_confirm")) {
			@Override
			public void effects() {
				Main.game.setParserTarget("prey", getMugger());
			}
			@Override
	...
	private static Response getVoreSwallowResponse() {
		if(!Main.game.isVoreContentEnabled()) {
			return null;
		}
		if(!Main.game.canBeSwallowed(getMugger(), "STOMACH")) {
			return new Response("吞噬",
					UtilText.parse(getMugger(), Main.game.getSwallowBlockReason(Main.game.getPlayer(), getMugger(), "STOMACH")),
					null);
		}
		return new Response("吞噬",
				UtilText.parse(getMugger(), "将[npc.name]整个人吞进你的胃里。"),
				DialogueManager.getDialogueFromId("unbirth_vore_swallow_confirm")) {
			@Override
			public void effects() {
				Main.game.setParserTarget("prey", getMugger());
			}
	...
	private static Response getUnbirthCarryingResponse() {
		if(Main.game.getContainedCharacterCount(Main.game.getPlayer())<=0) {
			return null;
		}
		return new Response("体内",
				"感受一下此刻被你容纳在体内的人，并决定是否要将他们释放出来。",
				DialogueManager.getDialogueFromId("unbirth_scenes_carrying")) {
			@Override
			public Colour getHighlightColour() {
				return PresetColour.GENERIC_SEX;
			}
	...
	public static final DialogueNode ALLEY_ATTACK = new DialogueNode("敌袭！", "一个家伙从阴影中跳了出来！", true) {
		@Override
		...
		public Response getResponse(int responseTab, int index) {
			...
			if(isWantsToFight()) {
				if (index == 1) {
					return new ResponseCombat("战斗", "挺身而出，与[npc.name]决一胜负！", getMugger()) {
						@Override
						public void effects() {
					...
					if(getMugger().isAttractedTo(Main.game.getPlayer())) {
						return new ResponseSex("献上身体",
								"向[npc.name]献上你的身体，以此来避免发生一场暴力对抗。"
					...
					if(!getMugger().isAttractedTo(Main.game.getPlayer())) {
						return new Response(UtilText.parse(companion, "三人行"),
								UtilText.parse(getMugger(), companion, "你看得出来，[npc.name]一点也不想和你做爱，所以也就不会想要和你三人行……"),
								null);
						
					} else if(!getMugger().isAttractedTo(companion)) {
						return new Response(UtilText.parse(companion, "三人行"),
								UtilText.parse(getMugger(), companion, "你看得出来，[npc.name]一点也不想和[npc2.name]做爱，所以这场三人行准是没戏了……"),
								null);
						
					} else if(!companion.isAttractedTo(getMugger()) && companion.isAbleToRefuseSexAsCompanion()) {
						return new Response(UtilText.parse(companion, "三人行"),
								UtilText.parse(getMugger(), companion, "你看得出来[npc2.name]完全没兴趣和[npc.name]做爱，而你也无法强迫[npc2.herHim]这么做……"),
								null);
						
					} else {
						return new ResponseSex(UtilText.parse(companion, "三人行"),
								UtilText.parse(getMugger(), companion, "让[npc.name]有和你，以及[npc2.name]一起做爱的机会，以此来避免一场暴力冲突。"),
								true, false,
								new SMGeneric(
										Util.newArrayListOfValues(getMugger()),
					...
					if(!getMugger().isAttractedTo(companion)) {
						return new Response(UtilText.parse(companion, "献上[npc.name]"),
								UtilText.parse(getMugger(), companion, "你看得出来[npc.name]完全没兴趣和[npc2.name]做爱……"),
								null);
						...
						return new ResponseSex(UtilText.parse(companion, "献上[npc.name]"),
								UtilText.parse(getMugger(), companion, "为了避免发生暴力冲突，你告诉[npc.name]，[npc.she]可以享用[npc2.namePos]的身体。"),
								true, false,
								new SMGeneric(
										Util.newArrayListOfValues(getMugger()),
				...
				if (index == 1) {
					if(isStorm()) {
						return new Response("对话",
								"[npc.Name]受到奥术风暴的影响，满脑子只想着和你做爱！",
								null);
					}
					return new Response("对话", "和[npc.name]聊一会儿，以便于多了解[npc.herHim]一点。", ALLEY_PEACEFUL_TALK) {
						@Override
						public void effects() {
							applyPregnancyReactions();
					...
					if(Main.game.getPlayer().getMoney()<Main.game.getDialogueFlags().getMuggerDemand1()) {
						...
						return new Response("献上财物("+UtilText.formatAsMoney(Main.game.getDialogueFlags().getMuggerDemand1(), "span")+")",
								"给[npc.name]一些钱，让[npc.herHim]能够买一些食物和衣服。", ALLEY_PEACEFUL_OFFER_MONEY) {
							@Override
							public void effects() {
								applyPregnancyReactions();
					...
					if(isStorm()) {
						...
						return new Response("提供房间", "问一问[npc.name]想不想在莉莱雅的宅邸中要一个房间。", ALLEY_PEACEFUL_OFFER_ROOM) {
							@Override
							public void effects() {
								applyPregnancyReactions();
					...
					return new Response("攻击", "背叛[npc.namePos]的信任并攻击[npc.herHim]！", ALLEY_PEACEFUL_ATTACK) {
						@Override
						public void effects() {
							applyPregnancyReactions();
					...
					if(!getMugger().isAttractedTo(Main.game.getPlayer())) {
						...
						return new ResponseSex(UtilText.parse(companion, "三人行"),
								UtilText.parse(getMugger(), companion, "让[npc.name]拥有和你，以及[npc2.name]一起做爱的机会。"),
								true, false,
								new SMGeneric(
										Util.newArrayListOfValues(getMugger()),
										Util.newArrayListOfValues(Main.game.getPlayer(), companion),
					...
					if(!getMugger().isAttractedTo(companion)) {
						...
						return new ResponseSex(UtilText.parse(companion, "献上[npc.name]"),
								UtilText.parse(getMugger(), companion, "告诉[npc.name][npc.she]可以使用[npc2.namePos]的身体。"),
								true, false,
								new SMGeneric(
										Util.newArrayListOfValues(getMugger()),
										Util.newArrayListOfValues(companion),
	...
	public static final DialogueNode ALLEY_PEACEFUL_TALK = new DialogueNode("对话", "", true, true) {
	...
	public static final DialogueNode ALLEY_PEACEFUL_OFFER_MONEY = new DialogueNode("献上金钱", "", true, true) {
	...
	public static final DialogueNode ALLEY_PEACEFUL_OFFER_ROOM = new DialogueNode("提供房间", "", true, true) {
		...
		public Response getResponse(int responseTab, int index) {
			if (index == 1) {
				return new Response("带回家", "带[npc.name]到[npc.her]的新房间。", ALLEY_PEACEFUL_OFFER_ROOM_BACK_HOME) {
					@Override
					public void effects() {
	...
	public static final DialogueNode ALLEY_PEACEFUL_OFFER_ROOM_BACK_HOME= new DialogueNode("新房间", "", true) {
	...
	public static final DialogueNode ALLEY_PEACEFUL_ATTACK = new DialogueNode("攻击", "", true, true) {
		...
		public Response getResponse(int responseTab, int index) {
			if (index == 1) {
				return new ResponseCombat("战斗", "开始与[npc.name]战斗！", getMugger());
			}
			return null;
		}
	};
	...
	public static final DialogueNode AFTER_SEX_PEACEFUL = new DialogueNode("继续", "从[npc.name]身边走开，准备继续你的旅程。", true) {
		...
		public Response getResponse(int responseTab, int index) {
			if (index == 1) {
				return new Response("继续", "继续走你的路。", Main.game.getDefaultDialogue(false));
			}
			return null;
		}
	};
			...
			if (index == 1) {
				return new Response("继续",
						"继续前行……"
							+ (getMugger().hasFlag(NPCFlagValue.genericNPCBetrayedByPlayer)
								?UtilText.parse(getMugger(), "<br/>[style.italicsBad([npc.Name]将会从游戏中永久移除。)]")
								:""),
						Main.game.getDefaultDialogue(false)){
					@Override
					public Colour getHighlightColour() {
				...
				return new Response("快速转化",
						"[npc.she]能够转化自己，你脑海中闪过一些点子……"
								+ "<br/><i>你完成[npc.herHim]的转化后将返回选项界面。</i>",
				...
				return new Response(
						"移除角色",
						UtilText.parse(getMugger(), "赶[npc.name]走。"
				...
				if(!Main.game.isNonConEnabled() && (!getMugger().isAttractedTo(Main.game.getPlayer()) || !getMugger().isAttractedTo(companion))) {
					return new Response("三人行", UtilText.parse(companion, getMugger(), "[npc2.Name]没兴趣和你或[npc.name]做爱！"), null);
					
				} else if(!companion.isAttractedTo(getMugger())) {
					return new Response(UtilText.parse(companion, "三人行"), UtilText.parse(companion, getMugger(), "[npc.Name]并没有[npc2.name]所吸引，所以不愿意跟[npc2.herHim]做爱！"), null);
				...
				if(!Main.game.isNonConEnabled() && !getMugger().isAttractedTo(companion)) {
					return new Response(UtilText.parse(companion, "交给[npc.name]"), UtilText.parse(companion, getMugger(), "[npc2.Name]并没有被[npc.name]所吸引，所以不愿意跟[npc.herHim]做爱！"), null);
					
				} else if(!companion.isAttractedTo(getMugger())) {
					return new Response(UtilText.parse(companion, "交给[npc.name]"), UtilText.parse(companion, getMugger(), "[npc.Name]并没有被[npc2.name]所吸引，所以不愿意跟[npc2.herHim]做爱！"), null);
					
				} else {
					return new ResponseSex(UtilText.parse(companion, "交给[npc.name]"),
							UtilText.parse(companion, getMugger(), "告诉[npc.name]，[npc.she]可以跟[npc2.name]找找乐子，你在一旁看着。"),
							false, false,
							new SMGeneric(
									Util.newArrayListOfValues(companion),
									Util.newArrayListOfValues(getMugger()),
				...
				if(!Main.game.isNonConEnabled() && !companion.isAttractedTo(getMugger())) {
					return new Response(UtilText.parse(companion, "献上[npc.name]"), UtilText.parse(companion, getMugger(), "[npc.Name]没兴趣和[npc2.name]做爱！"), null);
					
				} else if(!getMugger().isAttractedTo(companion)) {
					return new Response(UtilText.parse(companion, "献上[npc.name]"), UtilText.parse(companion, getMugger(), "[npc2.Name]没兴趣和[npc.name]做爱！"), null);
					...
					return new ResponseSex(UtilText.parse(companion, "献上[npc.name]"),
							UtilText.parse(getMugger(), companion, "告诉[npc.name][npc.she]可以使用[npc2.name]的身体。"),
							true, false,
							new SMGeneric(
									Util.newArrayListOfValues(getMugger()),
									Util.newArrayListOfValues(companion),
				...
				return getVoreSwallowResponse();
			} else if (index == 14) {
				return getUnbirthSwallowResponse();
			} else if (index == 9 && !getMugger().isAbleToSelfTransform()) {
				return getVoreSwallowResponse();
			} else if (index == 0) {
				return getUnbirthCarryingResponse();
			}
			return null;
		}
	};
		...
		if(potion!=null && forcedTF) {
			sb.append(UtilText.parse(getMugger(), target,
					"<p>"
						...
						+ "[npc.speech(真是个乖[npc2.girl]！我要把你变成我完美的"+getMugger().getPreferredBodyDescription("b")+"啊！)]"
					+ "</p>"));
		...
		public Response getResponse(int responseTab, int index) {
			...
			if((!forcedTF || potion==null)
				...
				if (index == 1) {
					...
					return new Response("命令吞下",
							UtilText.parse(getMainCompanion(),
								"告诉[npc.name]把"+(companionMultiplePotions?"药水吞下去":"药水吞下去")+"。"
								+(getMainCompanion().getFetishDesire(Fetish.FETISH_TRANSFORMATION_RECEIVING).isNegative()
				...
				if(getMugger().isAttractedTo(Main.game.getPlayer())) {
					if(getMugger().isAttractedTo(getMainCompanion())) { // Threesome sex:
						if (index == 1) {
							...
							return new ResponseSex("抵抗做爱",
									UtilText.parse(getMugger(), "[npc.Name]强行压住了你"+(companionSex?UtilText.parse(getMainCompanion(), "和[npc.name]"):"")+"……"),
									false, false,
									new SMGeneric(
											Util.newArrayListOfValues(getMugger()),
											Util.newArrayListOfValues(
					...
					if(getMugger().isWillingToRape()) {
						...
						return new Response(
								...
								UtilText.parse(getMugger(), getMainCompanion(), "看起来[npc2.name]打算拒绝和[npc.name]做爱。"),
								Main.game.getDefaultDialogue(false)) {
							@Override
							public void effects() {
