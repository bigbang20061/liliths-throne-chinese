	...
	protected static void spawnEnforcers() {
		...
		if(Math.random()<chanceOfNewEnforcers) {
			...
			for(String id : enforcerIds) {
				try {
					NPC enforcer = (NPC) Main.game.getNPCById(id);
					if(enforcer!=null && !enforcer.isContained()) { // 收容中的猎物不被巡逻队召回
	...
	protected static NPC getSlaveWantingToUseYouInDominion() {
		...
		for(String id : Main.game.getPlayer().getSlavesOwned()) {
			try {
				...
				if(!slave.isContained() // 收容中的猎物不参与主动求欢事件
...
//		System.out.println(hornySlaves.size() +" | " + slaves.size());
	...
	protected static Value<NPC, NPC> getSlaveUsingOtherSlaveInDominion() {
		...
		for(String id : Main.game.getPlayer().getSlavesOwned()) {
			try {
				NPC slave = (NPC) Main.game.getNPCById(id);
				if(!slave.isContained() // 收容中的猎物不参与奴隶性交事件
		...
		for(String id : Main.game.getPlayer().getSlavesOwned()) {
			try {
				NPC slave = (NPC) Main.game.getNPCById(id);
				if(!slave.isContained() // 收容中的猎物不参与奴隶性交事件
						&& slave.hasSlavePermissionSetting(SlavePermissionSetting.SEX_RECEIVE_SLAVES)
						&& slave.getSlaveJob(Main.game.getHourOfDay())==SlaveJob.IDLE
						&& slave.getLocationPlace().getPlaceType()!=PlaceType.SLAVER_ALLEY_SLAVERY_ADMINISTRATION
						&& slave.hasSlavePermissionSetting(SlavePermissionSetting.GENERAL_OUTSIDE_FREEDOM)) {
...
//		setEncounterDialogue(null, forceEncounter);
	...
	public static Response exploreArea() {
		return exploreArea("该区域");
	}

	public static Response exploreArea(String areaDescription) {
		return new ResponseEffectsOnly(
				"探索",
				"探索" + areaDescription + "。尽管你觉得不太可能找到什么东西，但至少不用无所事事地来回晃悠了……"){
			@Override
...
//		System.out.println("AVAILABLE: "+ItemType.OFFSPRING_MAP.isAbleToBeUsed(Main.game.getPlayer(), Main.game.getPlayer()));
		...
		if(!Main.game.getPlayer().hasItemType(ItemType.OFFSPRING_MAP)) {
			return new Response("后代地图",
							UtilText.parse("你没有后代地图..."
									+ "<br/><i>后代地图可以向市政厅的[vanessa.name]购买。</i>"),
					null);
			...
			return new ResponseEffectsOnly("后代地图",
